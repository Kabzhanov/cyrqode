...
- **Подпись** — два профиля: `ed25519-sha256-v1` (основной) и
  ...
  векторы и руководство: `sdk/python/buip_signing.py`, `test-vectors/signing.json`,
  `spec/signing.md`.
...
Обоснование решений: [ADR 0001](spec/adr/0001-crypto-and-identifiers.md).
Менять их после выпуска SDK нельзя — сторонние реализации перестанут сходиться.

**Версия 0.x — черновик:** схемы ещё могут меняться, в том числе ужесточаться.
Пишете свой SDK — привязывайтесь к конкретному commit, а не к «последней
версии». Правила совместимости, вступающие с 1.0: [spec/versioning.md](spec/versioning.md).

## Что не зафиксировано

Честно о состоянии, чтобы никто не строил на песке:

- **Формат физической марки** — геометрия, ёмкость и коррекция ошибок в
  ...
  процент читаемости повреждённой марки было бы враньём.
...
отдельно и распространяется на других условиях — см. [BOUNDARIES.md](BOUNDARIES.md).

Записи в `registry/` — **демонстрационные**. Данные реальных клиентов сюда не
попадают: они живут в базе сервиса.
